...
public class JwtAuthFilter extends OncePerRequestFilter {
	...
	try {
	    ...
	    log.warn("Token expired for request: {}", request.getServletPath());
	    sendError(response, "TOKEN_EXPIRED", "Token has expired");
	    return;
	} catch (JwtException e) {
	    log.warn("Invalid token for request: {}", request.getServletPath());
	    sendError(response, "INVALID_TOKEN", "Token is invalid");
	    return;
	}
	...
	if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
		...
		sendError(response, "INVALID_TOKEN", "Token is not valid");
		return;
	    }
